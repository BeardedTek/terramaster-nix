...
| `secrets/extra-files/persist/etc/lldap/ldap_user_pass` | only if `mySystem.features.sso.enable = true;` — copy `ldap_user_pass.example` in the same folder, fill in a real password (LLDAP's own bootstrap admin password, read once on first start — see `modules/lldap.nix`). LLDAP bootstraps its own JWT secret automatically, nothing to provide for that |
| `secrets/extra-files/persist/etc/authelia/{jwt_secret,session_secret,storage_encryption_key}` | only if `mySystem.features.sso.authelia.enable = true;` — copy each `.example` sibling, replace with a real `openssl rand -hex 32` value (one secret per file, matching Authelia's own convention, not `KEY=value`) |
| `secrets/extra-files/persist/etc/authelia/ldap_password` | only if `mySystem.features.sso.authelia.enable = true;` — copy `ldap_password.example`, fill in the password for the dedicated `uid=authelia,ou=people,...` bind account you create in LLDAP's own admin UI (see `modules/authelia.nix`) |
| `secrets/extra-files/persist/etc/opensmtpd/secrets` | only if `mySystem.smtp` is set — copy `secrets.example`, fill in the real upstream SMTP username/password (OpenSMTPD `table(5)` format: `relay username:password`, label must match `modules/smtp-relay.nix`). This relay is the only thing that ever holds the real credentials — every consumer (Authelia's notifier, and anything added later) talks to it unauthenticated over loopback instead. Leave `mySystem.smtp` unset to keep Authelia's file-based notification fallback (`/var/lib/authelia-main/notifications.txt`) instead |
| `secrets/extra-files/persist/etc/minio/minio.env`'s `MINIO_IDENTITY_OPENID_CLIENT_SECRET` | only if `mySystem.features.sso.authelia.enable = true;` — one more line in the same file as `MINIO_ROOT_USER`/`MINIO_ROOT_PASSWORD` above, the plaintext OIDC client secret matching the hash registered in `modules/authelia.nix`'s `candidateOidcClients."minio-console"` |
