{
  ...
  outputs = { self, nixpkgs, disko, impermanence, ... }:
    let
      ...
      # Same shape, for modules/dashboard-users.nix's add/modify/remove
      # flow — forces the whole mySystem.users list, not one field, since
      # that option has no smaller pieces to override independently.
      usersOverridesPath = /persist/nixos-users-overrides.nix;
        ...
        let variablesFile = variablesFileFor host; in
        {
          ...
          value = nixpkgs.lib.nixosSystem {
            ...
            modules = [
              ...
              ++ (if builtins.pathExists usersOverridesPath then [ usersOverridesPath ] else [ ]);
          };
        };
    in
    {
