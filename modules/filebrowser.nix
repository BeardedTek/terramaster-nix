...
let
  ...
  configFile = (pkgs.formats.yaml { }).generate "filebrowser-config.yaml" {
    ...
    # Full read/write for every newly-created (non-admin) user by
    # default — the OS-level permissions on /rust/media and
    # /rust/data (group-writable, 2775, filebrowser is in mediagroup
    # — see modules/media-stack.nix) already allow this, but Quantum
    # enforces its own separate, app-level permission model on top.
    # This is global, not per-source (this deployed version —
    # v1.4.0-stable, confirmed by reading
    # gtsteffaniak/filebrowser@v1.4.0-stable's own
    # backend/common/settings/structs.go/settings.go directly, since
    # the config schema has changed across versions and the package
    # docs/HEAD source don't necessarily match what's actually
    # installed — there is no per-source permission model at this
    # version at all, just this one global default). admin/api/share/
    # realtime deliberately left at their zero-value (false) — the
    # "admins" OIDC group (adminGroup above) already grants admin
    # access through a separate path, not this one. Only takes effect
    # for users created *after* this is set (ApplyUserDefaults only
    # runs in the auto-create-on-first-login branch) — an
    # already-existing non-admin account needs its permissions edited
    # once by hand (Settings -> Users) to pick this up.
