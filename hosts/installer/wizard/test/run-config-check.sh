...
# "path:" prefix: same reasoning as stages/90-install.sh's own — forces
# plain-directory evaluation regardless of whether $WIZ_REPO_WORKDIR
# happens to be a real git checkout, so this stays correct even if this
# cache dir is ever pointed at one.
if ! NIX_CONFIG="pure-eval = false" nix eval --impure --raw \
    "path:$WIZ_REPO_WORKDIR#nixosConfigurations.$hostname.config.system.build.toplevel.drvPath" >/dev/null; then
  echo >&2
