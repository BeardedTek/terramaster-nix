...
<div class="border border-gray-200 dark:border-gray-700 rounded-lg mb-3">
  ...
  <div id="services-group-meshvpn-panel" class="hidden border-t border-gray-200 dark:border-gray-700 p-3">
    <div class="flex items-center justify-between py-2">
      ...
      <label class="switch"><input type="checkbox" data-flag="nebula.enable" data-group-toggle="nebula"><span class="slider"></span></label>
    </div>
    <div class="pl-6 space-y-2 mb-2" data-group="nebula">
      <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">
        Paste a complete Nebula <code>config.yaml</code> &mdash; the same
        format the Nebula mobile apps use, with the CA/cert/key embedded
        directly in the file. Saving replaces the current config and
        restarts Nebula immediately; it does not require a system
        rebuild. The existing config is never shown back here (it
        contains this node's private key) &mdash; paste a full config
        to replace it.
      </p>
      <div id="nebula-config-status" class="text-xs text-gray-500 dark:text-gray-400 mb-2"></div>
      <textarea id="nebula-config" rows="10" placeholder="pki:&#10;  ca: ...&#10;  cert: ...&#10;  key: ...&#10;static_host_map:&#10;  ..." class="font-mono text-xs bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white"></textarea>
      <div id="nebula-config-message" class="mt-2 text-sm hidden"></div>
      <button id="nebula-config-save-btn" type="button" class="mt-2 text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Save Config</button>
    </div>
  </div>
</div>
...
<script>
(function () {
  var statusEl = document.getElementById("nebula-config-status");
  var textarea = document.getElementById("nebula-config");
  var messageEl = document.getElementById("nebula-config-message");
  var saveBtn = document.getElementById("nebula-config-save-btn");
  ...
  saveBtn.addEventListener("click", function () {
    ...
    if (!body.trim()) {
      showMessage("Paste a config.yaml first.", "error");
      return;
    }
